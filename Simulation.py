...
from SystemSerializer import SystemSerializer
...
class Simulation:
    ...
    def handleInput(self, input):
        ...
        if type == "String":
                ...
                if data == "Ready":
                    ...
                    try:
                        ...
                        central = {
                            ...
                            "Data" : self.system.centralBody.getJson()
                        }
                        centralJson = json.dumps(central)
                        updateStr += centralJson + "\n"

                        for b in self.system.orbitingBodies:
                            body = {
                                "ID_Receiver" : self.ids,
                                "ID_Sender" : self.id,
                                "Data_Type" : "JSONObject",
            ...
            if label == "Hand":
                ...
                handBody = Body(hand["name"], self.system.handMass, hand["scaleX"] / 2 * self.system.scale)
                handBody.setScale(self.system.scale)
                handBody.setLocation(Vector(hand["posX"], hand["posY"], hand["posZ"]), True)
                handBody.setVelocity(Vector(hand["velocityX"], hand["velocityY"], hand["velocityZ"]), True)
                self.system.updateBody(handBody)
                return
    ...
    def handleButtonInput(self, input):
        ...
        if input == "Speed Up":
            ...
            print "Unity wants to add a planet to the system. Prompting user for more information..."
            keyboard = {
                "ID_Receiver" : self.ids,
                "ID_Sender" : self.id,
                "Data_Type" : "JSONObject",
    ...
    def handleKeyboardInput(self, input):
        ...
        if input[1:11] == "Add planet":
            ...
            if param == "name":
                ...
                newBody = Body(self.newPlanetParams["name"], self.newPlanetParams["mass"], self.newPlanetParams["radius"])
    ...
    def loadSystem(self, filename):
        ...
        dCentral = {
            ...
            "Data_Label" : "Destroy",
            "Data" : self.system.centralBody.getJson()        }
        self.sock.sendall(json.dumps(dCentral) + "\n")
        ...
        with open(filename) as jsonFile:
            ...
            self.system = SystemSerializer.jsonToSystem(jsonArray)

    def saveSystem(self, filename, sys):
        with open(filename, 'w') as outFile:
            jsonArray = SystemSerializer.systemToJson(sys)
    ...
    def syncAndRun(self):
        ...
        while self.syncing:
            ...
            if self.system != None:
                toRemove = self.system.TickSystem(dt)
                ...
                for b in toRemove:
                    ...
                    self.sock.sendall(json.dumps(destroyMessage) + "\n")
                    print "Body", b.name, "from simulation", self.id, "removed from system because it exceeded the system boundary."
